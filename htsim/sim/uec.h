...
public:
    ...
    uint32_t findFreePort(const UecSrc* src = nullptr);
    void doNextEvent();
...
private:
    ...
    bool sourceHasFreePort(const UecSrc& src) const;
    const Route* routeForSourceIfFree(UecSrc& src);
    UecSrc* popNextActiveSourceWithFreePort();
...
public:
    ...
    UecSrc(TrafficLogger* trafficLogger, 
           ...
           bool rts = false);
    static constexpr uint32_t ANY_NIC_PORT = UINT32_MAX;
    ...
    void setPreferredNicPort(uint32_t port_num);
    uint32_t preferredNicPort() const { return _preferred_nic_port; }
