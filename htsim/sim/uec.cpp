...
const Route* UecNIC::requestSending(UecSrc& src) {
    ...
    const Route* route = routeForSourceIfFree(src);
    if (!route) {
        // We're already sending on all ports, or this flow is pinned to a
        // source rail whose physical port is currently busy.
        /*
...
bool UecNIC::sourceHasFreePort(const UecSrc& src) const {
    if (_busy_ports == _no_of_ports) {
        return false;
    }
    uint32_t preferred = src.preferredNicPort();
    if (preferred == UecSrc::ANY_NIC_PORT) {
        return true;
    }
    assert(preferred < _no_of_ports);
    return !_ports[preferred].busy;
}

uint32_t UecNIC::findFreePort(const UecSrc* src) {
    assert(_busy_ports < _no_of_ports);
    if (src && src->preferredNicPort() != UecSrc::ANY_NIC_PORT) {
        uint32_t preferred = src->preferredNicPort();
        assert(preferred < _no_of_ports);
        if (_ports[preferred].busy) {
            return _no_of_ports;
        }
        _rr_port = preferred;
        return preferred;
...
const Route* UecNIC::routeForSourceIfFree(UecSrc& src) {
    if (!sourceHasFreePort(src)) {
        return NULL;
    }
    uint32_t portnum = findFreePort(&src);
    if (portnum == _no_of_ports) {
        return NULL;
    }
    return src.getPortRoute(portnum);
}

UecSrc* UecNIC::popNextActiveSourceWithFreePort() {
...
void UecNIC::startSending(UecSrc& src, mem_b pkt_size, const Route* rt) {
    ...
    auto queued_it = find(_active_srcs.begin(), _active_srcs.end(), &src);
    if (queued_it != _active_srcs.end()) {
        _active_srcs.erase(queued_it);
    }
...
void UecNIC::cantSend(UecSrc& src) {
    ...
    if (!_active_srcs.empty()) {
        ...
        UecSrc* ready_src = popNextActiveSourceWithFreePort();
        if (ready_src) {
            // give the next src a chance.
            const Route* route = routeForSourceIfFree(*ready_src);
            assert(route);
            ready_src->timeToSend(*route);
            return;
        }
    }
    if (!_control.empty()) {
...
void UecNIC::doNextEvent() {
    ...
    if (!_active_srcs.empty() && !_control.empty()) {
        ...
        if (_crt < _ratio_data) {
            ...
            UecSrc* queued_src = popNextActiveSourceWithFreePort();
            if (queued_src) {
                const Route* route = routeForSourceIfFree(*queued_src);
                assert(route);
...
UecSrc::UecSrc(TrafficLogger* trafficLogger, 
    ...
    _preferred_nic_port = ANY_NIC_PORT;
...
void UecSrc::setPreferredNicPort(uint32_t port_num) {
    assert(port_num < _no_of_ports);
    _preferred_nic_port = port_num;
}
...
void UecSrc::timeToSend(const Route& route) {
    ...
    // The selected route also identifies the physical source port in
    // multi-port OXC mode.  Passing NULL here lets the NIC pick another
    // free port and breaks source-port pinning.
    if (bytes_sent > 0) {
    ...
    while (isSendPermitted()) {
        // We're ready to send again. Let the NIC know. In a single-port NIC,
        // the NIC will normally queue us and return NULL. In a multi-port NIC,
        // another port may be free immediately, so continue sending instead
        // of assuming the source must block.
        ...
        mem_b extra_bytes_sent = 0;
        if (_rtx_queue.empty()) {
            extra_bytes_sent = sendNewPacket(*newroute);
        } else {
            extra_bytes_sent = sendRtxPacket(*newroute);
        }
        if (extra_bytes_sent > 0) {
            _nic.startSending(*this, extra_bytes_sent, newroute);
        } else {
            _nic.cantSend(*this);
            return;
        }
