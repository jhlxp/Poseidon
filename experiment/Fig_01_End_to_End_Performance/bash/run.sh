...
if [[ "${REUSE_BASE}" == "1" ]]; then
    ...
    for hardware in H20 H100; do
        ...
        run_case "${hardware}_baselines" \
            python3 "${REPO_ROOT}/tests/run_dsv3_2layer_algorithms.py" \
            "${full_args[@]}" --workers 4 \
            --algorithms nccl,deepep,eplb,moonep \
            --num-layers 2 --gate-provider raw_receive_cdf \
            --gate-layer-map 0,1 --gate-seed 17 \
            --moonep-replicas-per-rank 256
        manifest_add "${hardware}_baselines" "${hardware}" "" \
            raw_receive_cdf "${profile}" 400 "${LAST_RUN}"

        run_case "${hardware}_probeep" \
        ...
        manifest_add "${hardware}_probeep" "${hardware}" probeep \
            raw_receive_cdf "${profile}" 400 "${LAST_RUN}"
    done
fi
